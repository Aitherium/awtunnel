...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awtunnel
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awtunnel-*.whl dist/*.tar.gz ...]
...
_KEYSTONES = ("awtunnel/__init__.py", "awtunnel/cli.py", "awtunnel/rules.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awtunnel-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awtunnel/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awtunnel/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awtunnel/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awtunnel/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awtunnel/y.py": "from awtunnel.client import call\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awtunnel-0.0.0.tar.gz"
